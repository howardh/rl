...
def print_frozen_lake_values(agent, f=features.one_hot):
...
def lstd_control():
    ...
    action_space = np.array([0,1,2,3])
    agent = LSTDAgent(action_space=action_space, num_features=16, discount_factor=0.99, features=features.one_hot)
    ...
    while True:
        ...
        if iters % 500 == 0:
            ...
            rewards = agent.test(e, 100)
            print("Iteration %d\t Rewards: %f" % (iters, np.mean(rewards)))
            print_frozen_lake_policy(agent, f=features.one_hot)
...
def lstd_tile_coding_control():
    ...
    action_space = np.array([0,1,2,3])
    agent = LSTDAgent(action_space=action_space, num_features=10, discount_factor=0.99, features=features.tile_coding)
    ...
    while True:
        ...
        if iters % 500 == 0:
            ...
            rewards = agent.test(e, 100)
            print("Iteration %d\t Rewards: %f" % (iters, np.mean(rewards)))
            print_frozen_lake_policy(agent, f=features.tile_coding)
            print_frozen_lake_values(agent, f=features.tile_coding)
            if np.mean(rewards) >= 0.78:
                break
