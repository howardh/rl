...
from frankenstein.buffer.shared import SharedBuffer, SharedBufferVec
...
def squeeze(x, dim):
    if isinstance(x, torch.Tensor):
        ...
        return {k: squeeze(v, dim) for k,v in x.items()}
    elif isinstance(x, Tuple):
        return tuple(squeeze(v, dim) for v in x)
    else:
        raise NotImplementedError(f"Unknown data type: {type(x)}")
...
class ImpalaTrainer:
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                if isinstance(observation_space, gym.spaces.Dict):
                    obs = {k:v for k,v in obs.items() if k not in self.ignore_obs}
                ...
                buffer.append_obs(
                        ...
                        misc = {
                            ...
                            'state_value': output['state_value'].item(),
                            'action_log_prob': action_log_prob.item(),
                            'task': task_id,
                            'hidden': squeeze(output['hidden'], 1),
                        }
                )
    ...
    def load_state_dict(self, state_dict):
        ...
        if 'logger' in state_dict:
            self.logger.load_state_dict(state_dict['logger'], wandb_method='copy')


# Attempts to speed up IMPALA below

Batch = namedtuple('Batch', ['obs', 'action', 'reward', 'terminal', 'misc'])


class ImpalaTrainer2(ImpalaTrainer):
    """
    Implementation of Impala. See https://arxiv.org/abs/1802.01561.
    ...
    @staticmethod
    def _action_dist(action_space, output):
        if isinstance(action_space, gym.spaces.Box):
            return torch.distributions.Normal(output['action_mean'], output['action_std'])
        elif isinstance(action_space, gym.spaces.Discrete):
            return torch.distributions.Categorical(logits=output['action'])
    ...
    @staticmethod
    def _actor_loop(env, task_id, net, buffer, seed=None):
        if seed is None:
            seed = os.getpid() + int(time.time())
            thread_id = threading.current_thread().ident
            if thread_id is not None:
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                output = net(unsqueeze(to_tensor(obs, device), 0), hidden)
                action_dist = ImpalaTrainer2._action_dist(env.action_space, output)
                action = action_dist.sample()
                action_log_prob = action_dist.log_prob(action)
                action = action.item()
                hidden = output['hidden']
    ...
    def _batch_loop(self, buffer, batch_size, queue):
        while self._steps < self.total_steps:
    ...
    def _train_loop(self, shared_net, net, buffer, batch_queue, optimizer, batch_size, action_space, profiler):
        while self._steps < self.total_steps:
            profiler.enable()
            print('Waiting for batch')
            batch, indices = batch_queue.get()
            print('batch received')
            batch = Batch(*[to_tensor(b, self.device) for b in batch])
    ...
    def train(self):
        ...
        # Start training processes/threads
        profilers = [cProfile.Profile() for _ in range(self.num_train_loop_workers)]
        batch_queue = ctx.SimpleQueue()
        batch_process = ctx.Process(
...
class ImpalaTrainer3:
    ...
    def _batch_actor_loop(self, env_configs, task_ids, net, buffer, step_count, seed=None):
        net.to(self.device)
        if seed is None:
            seed = os.getpid() + int(time.time())
            thread_id = threading.current_thread().ident
        ...
        env = make_vec_env(env_type='gym_async', env_configs=env_configs)
        ...
        with torch.no_grad():
            ...
            episode_step_count = np.zeros(num_envs, dtype=np.int32)
            ...
            hidden = net.init_hidden(num_envs)

            while True:
                if done.any():
                    episode_step_count[done] = 0
                    episode_return[done] = 0

                episode_step_count += 1
                episode_return += reward

                output = net(to_tensor(obs, device), hidden)
                action_dist = self._action_dist(env.single_action_space, output)
                action = action_dist.sample()
                action_log_prob = action_dist.log_prob(action)
                ...
                buffer.append_obs(
                        ...
                        misc = {
                            ...
                            'state_value': output['state_value'],
                            'action_log_prob': action_log_prob,
                            'task': torch.tensor(task_ids),
                            'hidden': output['hidden'],
                        }
                )

                buffer.append_action(action.reshape(-1,1))
    ...
    def train(self):
        ...
        buffer = SharedBufferVec(
                ...
                batch_size = len(self._env_configs)
        )

        step_count = torch.tensor(0).share_memory_()
        #self._batch_actor_loop(self._env_configs, [0]*len(self._env_configs), self.net, buffer, step_count)
        actor_process = ctx.Process(
            target=self._batch_actor_loop,
            args=(
                self._env_configs,
                [0]*len(self._env_configs),
                self.net,
                buffer,
                step_count
            )
        ...
        try:
            while self._steps < self.total_steps:
                time.sleep(5)
                sps = self._steps / (timer() - start_time)
                #sps = step_count.item() / (timer() - start_time)
                print(f'{self._steps:,} steps -- {sps:.2f} steps/sec')
        except KeyboardInterrupt:
            pass
...
def train_mp5():
    ...
    envs = [make_env(**conf) for conf in env_config]
    ...
    trainer = ImpalaTrainer(
            ...
            lr_scheduler = {
                ...
                'parameters': {
                    'start_factor': 1.0,
                    'end_factor': 0,
                    'total_iters': 200_000_000/8/128, # Total steps / batch size / rollout length
                }
            },
