...
def get_epsilon_greedy(epsilon):
    def f(w):
        m = np.argmax(w)
...
class Learner(object):
    ...
    def __init__(self):
        ...
        self.target_policy = get_epsilon_greedy(0)
        self.behaviour_policy = get_epsilon_greedy(0.1)
    ...
    def get_behaviour_policy(self, state):
        values = self.get_all_state_action_values(state)
        return self.behaviour_policy(values)
